  ...
  <div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <h1 class="text-4xl lg:text-5xl font-extrabold text-gray-900 leading-tight mb-6">
              Los mejores combos <span class="text-transparent bg-clip-text bg-gradient-to-r from-[#E94E1B] to-[#F4A261]">orientales</span>
            </h1>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div v-else-if="error" class="text-center py-12">
          <div class="text-red-500 mb-4">
            ...
            <button @click="refresh" class="mt-4 bg-[#E94E1B] text-white px-6 py-2 rounded-xl hover:bg-[#c43d15] transition-colors font-medium">
              Intentar de nuevo
            </button>
          </div>
        </div>
        ...
        <div v-else class="text-center py-12">
          ...
          <p class="text-lg text-gray-500">No hay productos disponibles en este momento</p>
          <p class="text-gray-400 mt-2">¡Vuelve pronto! Estamos preparando nuevos productos para ti.</p>
        </div>
      </div>
    </section>
    ...
    <section id="combos" class="py-16 bg-gray-50">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Estado: Combos encontrados -->
        <div v-else-if="combos.length > 0" class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <ComboCard v-for="(combo, i) in combos" :key="combo.id" :combo="combo" :index="i" />
        </div>

        <!-- Estado: Sin combos -->
        <div v-else-if="!loading" class="text-center py-12">
          <Store class="w-16 h-16 mx-auto text-gray-300 mb-4" />
          <p class="text-lg text-gray-500">No hay combos disponibles en este momento</p>
          <p class="text-gray-400 mt-2">Estamos preparando nuevas ofertas para ti.</p>
        </div>
      </div>
    </section>
...
import { CheckCircle, Zap, DollarSign, ShoppingCart, ChevronDown, Truck, Package, Store, AlertCircle } from 'lucide-vue-next';
import { useCartStore } from '~/stores/cart';
import { useApi } from '~/composables/useApi';

const { products, combos, loading, error, refresh } = useApi();
